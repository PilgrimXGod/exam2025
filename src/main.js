// Файл: src/main.js (Фінальна версія для PC-симуляції, все в одному файлі)
...
async function main() {
    ...
    // 5. Керування мишкою
    // Цей виклик тепер гарантовано спрацює, бо defer в HTML чекає завантаження всіх скриптів
    controls = new THREE.OrbitControls(camera, renderer.domElement);
    controls.enableDamping = true; // Плавність руху
    controls.target.set(0, 0.5, 0); // Націлюємо камеру трохи вище центру
    controls.update();
...
function placeScene() {
    ...
    loader.load('./assets/models/server_rack.glb', (gltf) => {
        ...
        serverRackModel.scale.set(0.15, 0.15, 0.15); // Зробимо стійку трохи більшою
...
function addContainers(count) {
    ...
    loader.load('./assets/models/docker_whale.glb', (gltf) => {
        for (let i = 0; i < count; i++) {
            ...
            const angle = (containers.length / 10) * Math.PI * 2 + Math.random() * 0.5; // Додамо трохи хаосу
            const radius = 0.5 + Math.random() * 0.2; // Різна відстань
            ...
            container.rotation.y = Math.random() * Math.PI * 2; // Випадковий поворот
...
function render() {
    controls.update(); // Оновлюємо камеру при обертанні мишкою
    handleSimulation();
